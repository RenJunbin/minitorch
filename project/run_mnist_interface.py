...
def render_run_image_interface():
    ...
    if st_train_button.button("Train Model"):
        ...
        '''default_log_fn from run_mnist_multiclass.py
        ...
        def log_fn(epoch, total_loss, correct, total, losses, model):
            ...
            df.append({"epoch": epoch, "loss": total_loss, "correct": correct, "valid acc": correct/total})
            st_epoch_stats.write(pd.DataFrame(reversed(df)))
            ...
            st_epoch_plot.plotly_chart(fig)
            print(
                f"Epoch: {epoch}/{max_epochs}, loss: {total_loss}, correct: {correct}, valid acc: {correct/total}"
            )
